...
2. For each of the services (GitHub, GitLab, Jira, Figma, generic webhooks) fill in the respective variables `matrix_hookshot_service_*` listed in [main.yml](/roles/matrix/matrix-bridge-hookshot/defaults/main.yml) as required.
...
Other configuration options are available via the `matrix_hookshot_configuration_extension_yaml` and `matrix_hookshot_registration_extension_yaml` variables, see the comments in [main.yml](/roles/matrix/matrix-bridge-hookshot/defaults/main.yml) for how to use them.
...
See also `matrix_hookshot_matrix_nginx_proxy_configuration` in [init.yml](/roles/matrix/matrix-bridge-hookshot/tasks/inject_into_nginx_proxy.yml).

The different listeners are also reachable *internally* in the docker-network via the container's name (configured by `matrix_hookshot_container_url`) and on different ports (e.g. `matrix_hookshot_appservice_port`). Read [main.yml](/roles/matrix/matrix-bridge-hookshot/defaults/main.yml) in detail for more info.
...
- copy the *contents* of the downloaded file and set the variable `matrix_hookshot_github_private_key` to the contents (see example in [main.yml](/roles/matrix/matrix-bridge-hookshot/defaults/main.yml)).
...
```
For more info see the documentation in the [matrix-aux base configuration file](/roles/matrix/matrix-aux/defaults/main.yml).
