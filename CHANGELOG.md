...
- those that live within the playbook itself (`roles/matrix/*`)
...
Some variable names will change during the transition to having more and more external (galaxy) roles. There's a new `matrix/matrix_playbook_migration` role added to the playbook which will tell you about these changes each time you run the playbook.
...
To enable this module (and prevent encryption from being used on your homserver), add `matrix_synapse_ext_encryption_disabler_enabled: true` to your configuration. This module provides further customization. Check its other configuration settings (and defaults) in `roles/matrix/matrix-synapse/defaults/main.yml`.
...
- or, do the migration "steps" manually:
  ...
  - define `matrix_mautrix_facebook_database_*` variables in your `vars.yml` file (credentials, etc.) - you can find their defaults in `roles/matrix/matrix-mautrix-facebook/defaults/main.yml`
...
Each bridge now lives in its own separate role (`roles/matrix/matrix-bridge-*`).
...
Refer to the [default variables file](roles/matrix/matrix-mxisd/defaults/main.yml) for more information.
...
See the `matrix_riot_web_homepage_` variables in `roles/matrix/matrix-riot-web/defaults/main.yml`.
...
The [LDAP identity store for mxisd](https://github.com/kamax-matrix/mxisd/blob/master/docs/stores/ldap.md) can now be configured easily using playbook variables (see the `matrix_mxisd_ldap_` variables in `roles/matrix/matrix-server/defaults/main.yml`).
